...
module.exports = {
    ...
    .addUserOption(option => option.setName('user').setDescription('The user you would like to time out').setRequired(true))
    ...
    .addStringOption(option => option.setName('reason').setRequired(true).setDescription('The reason for timing out the user')),
    async execute(interaction, message, client) {
        ...
        await timeMember.timeout(duration * 1000, reason)
            ...
            .setDescription(`You have been timed out in ${interaction.guild.name}. You can check the status of your timeout within the server. | ${reason}`)
            .setColor(`Black`)
